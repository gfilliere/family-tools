...
const SIZE_WORDS = "(?:large|small|medium|big|heaped|heaping|level|whole|generous|good|scant|thick|thin|fat|juicy|ripe|fresh|large|grosse|grosser|kleine|kleiner|gehäufte|gehäufter|gestrichene|gestrichener|dicke|dünne)";
...
function detectFlags(text: string): LineFlags {
  ...
  return {
    optional: /\boptional\b|\bnach belieben\b|\bif desired\b|\bif you like\b|\bwenn gewünscht\b/.test(lower),
    toTaste: /\bto taste\b|\bnach geschmack\b|\bas needed\b|\bas required\b|\bnach bedarf\b|\betwas\b|\ba little\b|\ba bit\b|\bsome\b|\blots of\b|\bplenty of\b|\bgenerous pinch\b|\bpinch of\b|\bprise\b|\bsplash\b|\bschuss\b|\bdrizzle\b|\bdash\b|\bsprinkle\b/.test(lower),
    serving: /\bto serve\b|\bfor serving\b|\bzum servieren\b|\bto garnish\b|\bfor garnish\b|\bfor the table\b|\bon the side\b|\bals beilage\b|\bzum garnieren\b/.test(lower),
  };
}
...
function readHead(text: string): Head {
  ...
  if (match) {
    ...
    if (!unit) {
      const later = new RegExp(`^${SIZE_WORDS}?\\s*(${UNIT_PATTERN})\\b(?!\\s*(?:pieces?|stücke?))\\s*(?:of\\s+|von\\s+)?`, "i").exec(rest);
  ...
  const words = /^((?:a|an|one|two|three|four|five|six|seven|eight|nine|ten|dozen|half|quarter|couple|few|ein|eine|einen|einer|zwei|drei|vier|fünf|sechs|halbe|halben|halber|einige)\b(?:\s+(?:a|an|of|dozen))?)\s*/i.exec(cleaned);
  if (words) {
    ...
    const later = new RegExp(`^${SIZE_WORDS}?\\s*(${UNIT_PATTERN})\\b\\s*(?:of\\s+|von\\s+)?`, "i").exec(rest);
    if (later) { unit = unitOf(later[1]); rest = rest.slice(later[0].length); }
...
export function parseIngredientLine(original: string): ParsedLine {
  ...
  let text = original.normalize("NFKC").replaceAll("⁄", "/").replace(/(\d) \/ (\d)/g, "$1/$2").replace(/(\d),(\d{3})(?!\d)/g, "$1$2").replace(/\s+/g, " ").trim();
